...
class Modules():
	...
	def save_lcdata(self, ctr_list, ratio, savepath, filename):
		""" Save the cooridnate of the center point of bounding boxes """

		file = open(savepath + "center" + filename[:-4] + '.txt', 'w')

		f_line = "num\tcX(mm)\tcY(mm)\n"
		file.write(f_line)
		num = 0

		for (cX, cY) in ctr_list:
			cX_mm, cY_mm = cX * ratio, cY * ratio
			data = "%d\t%d\t%d\n" % (num + 1, cX_mm, cY_mm)
			file.write(data)
			num += 1
	...
	def save_plantdata(self, image, ratio, location_list, savepath, filename, f_type):
		file = open(savepath + "plant_" + filename[:-4] + '.txt', 'w')
		...
		for (x, y, w, h) in location_list:
			avgX, topY = x + (w / 2), y - h // 3 

			cv2.circle(image, (int(avgX), int(topY)), 2, (47, 173, 255), 2)

			avgX_mm, topY_mm = avgX * ratio, topY * ratio
			data = "%d\t%d\t%d\n" % (num + 1, avgX_mm, topY_mm)
			file.write(data)
			num += 1
	...
	def extract_data(self, location_list, ratio, image, savepath, filename, f_type):
		...
		# ctr_mm_list = self.transform_unit(ctr_list, ratio)
		if f_type == "v":
			image = self.save_plantdata(image, ratio, location_list, savepath, filename, f_type)
		self.save_lcdata(ctr_list, ratio, savepath, filename)

		return image

	def im_write(self, image, save_path, filename):
		cv2.imwrite(save_path + filename[:-4] + "_result.png", image)
