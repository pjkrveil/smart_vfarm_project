...
class Modules():
	...
	def transform_unit(self, ctr_list, ratio):

		ctr_mm_list = []

		# for (i, c) in enumerate(ctr_list):
		# 	if i == 0:
		# 		(prev_cx, prev_cy) = c
		# 	else:
		# 		min_dist = sys.maxsize

		# 		for j in range(i, len(ctr_list)):
		...
		# 	else:
		# 		if prev_cri == curr_cri:

		# 			x_mm_dist = (curr_px - prev_px) * ratio
		# 			y_mm_dist = (curr_py - prev_py) * ratio

		# 			ctr_mm_list.append((x_mm_dist, y_mm_dist))
		...
		for (i, c) in enumerate(ctr_list):
			if i == 0:
				...
				x_mm_dist = (curr_cx - prev_cx) * ratio
				y_mm_dist = (curr_cy - prev_cy) * ratio
	...
	def extract_data(self, location_list, ratio, image, filename):
		...
		self.save_lcdata(ctr_mm_list, filename)
